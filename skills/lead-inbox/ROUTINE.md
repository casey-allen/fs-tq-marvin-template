...
2. Gmail: find labeled-not-logged threads (search above). Pull full thread content for each.
   **Backlog guard:** process at most **5 threads per run, NEWEST first** — the rest catch the next run (stamping makes this safe; a 55-min run died trying to do 15, so keep runs small and finish clean). **Time-box yourself:** finish each thread completely (row → card → stamp) before starting the next, and if you've been working ~15 minutes, stop after the current thread and let the next run continue — a clean partial run beats a dead complete one. If more than 3 of a run's threads are dedup hits (already tracked), post ONE digest message listing them (`Backfilled N already-tracked leads: L-0xx {name}, …`) instead of individual cards; reserve cards for genuinely new leads.
